...
def _mastery_turn_directive(context: UnifiedContext) -> str:
    ...
    if language.startswith("zh"):
        return (
            ...
            "完成一个默认 multiple-choice quick quiz 卡片；早期 quick check 答对后不要再追问同一"
            "目标的自由复述，除非 lesson contract 明确要求。\n"
            "- 如果 lesson contract 的 `visual_aid.required` 为 true，先调用 mastery_visual，并把"
            "返回的 markdown 字段原样放入面向学习者的讲解，包括 ```mermaid 起止围栏；不要改写、"
            "内联或部分重写图示语法，也不要只用文字替代。\n"
            "- 对 memory/procedure：使用 mastery_quiz + ask_user 出题；不要把题目做成普通文本菜单，"
            "也不要让学习者提供 knowledge_point_id、题目或选项。\n"
            "- 如果 trusted/local grounding 提供了 scenario、choices、answer 或 expected_answer，"
            "必须逐字使用这些本地题干、选项和答案注册 mastery_quiz；不要改写数字、序列、选项或"
            "正确答案。若需要变体，必须重新计算答案且通过 mastery_quiz 校验。\n"
    ...
    return (
        ...
        "quick quiz card. After a correct early quick check, do not ask a second "
        "free-response explanation for the same objective unless the lesson "
        "contract explicitly requires it.\n"
        "- If the lesson contract says `visual_aid.required=true`, call mastery_visual "
        "first and copy its returned markdown field exactly into the learner-facing "
        "explanation, including the opening and closing ```mermaid fences. Do not "
        "paraphrase, inline, or partially rewrite diagram syntax, and do not replace "
        "it with text-only teaching.\n"
        ...
        "- If trusted/local grounding provides scenario, choices, answer, or "
        "expected_answer, register mastery_quiz with those local question words, "
        "options, and answers exactly. Do not rewrite numbers, sequences, options, "
        "or the correct answer. If you need a variant, recompute it and let "
        "mastery_quiz validate the answer.\n"
