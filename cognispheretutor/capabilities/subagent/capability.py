...
(resolved by :mod:`cognispheretutor.capabilities.subagent.binding`). As a
...
from cognispheretutor.capabilities.protocol import KnowledgeCapability, PromptBlock
from cognispheretutor.capabilities.subagent.binding import connection_for_turn
from cognispheretutor.capabilities.subagent.tools import SUBAGENT_TOOL_NAMES
...
class SubagentCapability(KnowledgeCapability):
    ...
    def augment_kwargs(
        ...
        if conn is None:
            return kwargs
        from cognispheretutor.services.subagent import load_subagent_settings
        from cognispheretutor.services.subagent.sessions import get_session, session_key
        ...
        # session the user/cognisphereTutor built up earlier (and the sidebar shares).
...
_DEFAULT_CONSULT_INSTRUCTION = (
    "You are being consulted programmatically by cognisphereTutor on the user's behalf, "
...
def _resolve_budget(context: UnifiedContext) -> int:
    ...
    """
    from cognispheretutor.services.subagent import load_subagent_settings
    from cognispheretutor.services.subagent.config import CONSULT_BUDGET_MAX, CONSULT_BUDGET_MIN
...
def _system_text(language: str, name: str, budget: int, kind: str = "") -> str:
    from cognispheretutor.services.subagent import PARTNER_BACKEND_KIND
