...
from cognispheretutor.capabilities.mastery import MASTERY_TOOL_TYPES
from cognispheretutor.capabilities.obsidian import OBSIDIAN_TOOL_TYPES
from cognispheretutor.capabilities.solve import SOLVE_TOOL_TYPES
from cognispheretutor.capabilities.subagent import SUBAGENT_TOOL_TYPES
from cognispheretutor.core.tool_protocol import BaseTool, ToolDefinition, ToolParameter, ToolResult
from cognispheretutor.tools.exec_tool import ExecTool
from cognispheretutor.tools.media_gen_tool import ImagegenTool, VideogenTool
...
from cognispheretutor.tools.prompting import load_prompt_hints
...
class BrainstormTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from cognispheretutor.tools.brainstorm import brainstorm
...
class WebSearchTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from cognispheretutor.tools.web_search import web_search
...
class CodeExecutionTool(_PromptHintsMixin, BaseTool):
    ...
    it through :mod:`cognispheretutor.services.sandbox`. No second LLM call, and the
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        from cognispheretutor.services.sandbox import (
            ExecRequest,
            Mount,
            ResourceLimits,
            get_sandbox_service,
        )
        from cognispheretutor.services.sandbox.artifacts import (
...
class ReasonTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from cognispheretutor.tools.reason import reason
...
class PaperSearchToolWrapper(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from cognispheretutor.tools.paper_search_tool import ArxivSearchTool
...
class GeoGebraAnalysisTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from cognispheretutor.agents.vision_solver.vision_solver_agent import VisionSolverAgent
        from cognispheretutor.services.llm.config import get_llm_config
...
class ReadMemoryTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from cognispheretutor.services.memory import get_memory_store
...
class WebFetchTool(_PromptHintsMixin, BaseTool):
    ...
    ``cognispheretutor.tools.web_fetch`` so this wrapper stays free of network
...
class ListNotebookTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from cognispheretutor.tools.list_notebook import list_notebooks_or_records
...
class WriteNoteTool(_PromptHintsMixin, BaseTool):
    ...
    * ``edit`` — patch an existing record's title / body / summary.
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from cognispheretutor.tools.write_note import write_note
...
class GithubTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from cognispheretutor.tools.github_query import run_github_query
...
class ReadSkillTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from cognispheretutor.services.skill import get_skill_service
