...
def _concat_l3() -> str:
    ...
    """
    from cognispheretutor.services.memory import get_memory_store, paths
...
def _resolve_partner_id() -> str | None:
    ...
    from cognispheretutor.services.partners.scope import PARTNER_USER_PREFIX
...
class PartnerReadTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from cognispheretutor.multi_user.paths import (
            get_admin_path_service,
            get_current_path_service,
        )
        from cognispheretutor.services.memory import memory_path_service_override
...
class PartnerMemorizeTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from cognispheretutor.multi_user.paths import get_current_path_service
        from cognispheretutor.services.memory import get_memory_store, memory_path_service_override
