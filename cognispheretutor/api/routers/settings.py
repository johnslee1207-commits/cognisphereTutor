...
from cognispheretutor.multi_user.model_access import allowed_llm_options
from cognispheretutor.services.config import (
    get_config_test_runner,
    get_model_catalog_service,
    get_runtime_settings_service,
)
from cognispheretutor.services.config.origins import normalize_origins
...
from cognispheretutor.services.embedding.client import reset_embedding_client
from cognispheretutor.services.llm.client import reset_llm_client
from cognispheretutor.services.llm.config import clear_llm_config_cache
from cognispheretutor.services.model_selection import list_llm_options
from cognispheretutor.services.path_service import get_path_service
from cognispheretutor.tools.builtin import USER_TOGGLEABLE_TOOL_NAMES
...
def get_enabled_optional_tools() -> list[str]:
    ...
    """
    from cognispheretutor.multi_user.tool_access import allowed_optional_tools
...
def _provider_choices() -> dict[str, list[dict[str, str]]]:
    ...
    from cognispheretutor.services.provider_registry import PROVIDERS
...
def _mineru_settings_payload() -> dict[str, Any]:
    ...
    """
    from cognispheretutor.services.parsing.engines.mineru.backend import local_cli_probe
...
def _document_parsing_payload() -> dict[str, Any]:
    ...
    from cognispheretutor.services.parsing.engines.factory import (
        get_parser,
        list_engines,
    )
    from cognispheretutor.services.parsing.engines.mineru.backend import local_cli_probe
...
async def test_document_parsing(payload: DocumentParsingTest):
    ...
    _require_settings_admin()
    from cognispheretutor.services.parsing.engines.factory import get_parser, is_engine_available
    ...
    if not cmd:
        return {
            ...
            "message": (
                ...
                f"(pip install cognispheretutor[parse-{engine}]) so its CLI is on PATH."
            ),
        }
...
async def start_mineru_models_download(payload: MinerUModelDownloadPayload):
    ...
    """
    _require_settings_admin()
    from cognispheretutor.services.parsing.engines.mineru.models import (
...
async def mineru_models_download_status(cursor: int = 0):
    _require_settings_admin()
    from cognispheretutor.services.parsing.engines.mineru.models import get_model_download_manager
...
async def test_mineru_connection(payload: MinerUSettingsUpdate):
    ...
    _require_settings_admin()
    from cognispheretutor.services.parsing.engines.mineru.cloud import verify_credentials
    from cognispheretutor.services.parsing.engines.mineru.config import MinerUConfig, MinerUError

    if payload.mode == "local":
        from cognispheretutor.services.parsing.engines.mineru.backend import (
            local_cli_probe,
            local_cli_version,
        )
...
async def fetch_models_from_provider(payload: FetchModelsPayload):
    ...
    """
    _require_settings_admin()
    from cognispheretutor.services.llm.factory import fetch_models as fetch_llm_models
...
async def complete_tour(payload: TourCompletePayload | None = None):
    ...
    return {
        "status": "completed",
        "message": "Configuration saved. cognisphereTutor will restart shortly.",
        "launch_at": launch_at,
