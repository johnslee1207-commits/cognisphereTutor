...
from cognispheretutor.api.routers.auth import require_admin
from cognispheretutor.knowledge.kb_types import SUBAGENT_KB_TYPE
from cognispheretutor.multi_user.knowledge_access import current_kb_manager
from cognispheretutor.multi_user.partner_access import assert_partner_allowed, visible_partner_cards
from cognispheretutor.services.rag.linked_kb import assert_path_allowed
...
async def backend_options():
    ...
    from cognispheretutor.services.subagent.models import list_backend_options
...
async def sync_backend(kind: str):
    ...
    from cognispheretutor.services.subagent.models import sync_backend_options
...
async def create_connection(payload: ConnectSubagentRequest):
    ...
    consult within one cognisphereTutor chat lands in that one partner session.
    """
...
async def delete_connection(name: str):
    ...
    from cognispheretutor.services.subagent.sessions import forget_connection
...
async def message_connection(name: str, payload: SubagentMessageRequest):
    ...
    live session cognisphereTutor consults (shared via the cross-turn registry, keyed by
