...
from cognispheretutor.partners.config.paths import get_partner_media_dir
from cognispheretutor.partners.helpers import safe_filename
from cognispheretutor.services.partners import (
    get_partner_manager,
    slugify_partner_id,
    slugify_soul_id,
)
...
def _validate_llm_selection_payload(
    ...
    from cognispheretutor.services.model_selection import apply_llm_selection_to_catalog
    from cognispheretutor.services.partners.model_runtime import normalize_partner_llm_selection
...
def _resolve_soul_content(soul: SoulSpec | None) -> tuple[str, dict[str, str]]:
    ...
    from cognispheretutor.services.partners.workspace import DEFAULT_SOUL
...
async def tool_options():
    ...
    """
    from cognispheretutor.api.utils.tool_options import build_tool_options
...
async def start_partner(partner_id: str):
    ...
    # comes back in this state after a cognisphereTutor restart (a manual /stop clears
...
async def partner_command_palette():
    from cognispheretutor.services.partners.commands import partner_command_palette
...
def _partner_upload_caps() -> tuple[int, int]:
    try:
        from cognispheretutor.services.config.runtime_settings import get_chat_attachment_limits
