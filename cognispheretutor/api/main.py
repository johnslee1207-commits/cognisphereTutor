...
from cognispheretutor.logging import configure_logging
...
def validate_tool_consistency():
    ...
    """
    try:
        from cognispheretutor.runtime.registry.capability_registry import get_capability_registry
...
async def lifespan(app: FastAPI):
    ...
    try:
        from cognispheretutor.services.llm import get_llm_client
    ...
    try:
        from cognispheretutor.events.event_bus import get_event_bus
    ...
    try:
        from cognispheretutor.services.partners import get_partner_manager
    ...
    try:
        from cognispheretutor.services.cron import get_cron_service
    ...
    try:
        from cognispheretutor.services.pocketbase_client import ping_pocketbase
    ...
    try:
        from cognispheretutor.services.memory import (
...
app = FastAPI(
    ...
    # See: https://github.com/HKUDS/cognisphereTutor/issues/112
...
# ones worth seeing.
#
# The `cognispheretutor.access` logger gets its own INFO stdout handler rather than
...
_access_logger = logging.getLogger("cognispheretutor.access")
if not any(getattr(h, "_cognispheretutor_access_handler", False) for h in _access_logger.handlers):
    ...
    _access_handler._cognispheretutor_access_handler = True  # type: ignore[attr-defined]
...
from cognispheretutor.multi_user.router import router as multi_user_router  # noqa: E402
...
from cognispheretutor.api.routers.auth import require_admin, require_auth  # noqa: E402
...
@app.get("/")
async def root():
    return {"message": "Welcome to cognisphereTutor API"}


if __name__ == "__main__":
    from cognispheretutor.api.run_server import main as run_server_main
