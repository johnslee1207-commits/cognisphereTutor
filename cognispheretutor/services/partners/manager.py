...
Each partner runs as a set of asyncio tasks within the cognisphereTutor server
...
from cognispheretutor.partners.config.paths import (
    get_data_dir,
    get_partner_dir,
...
def _slugify_id(name: str, *, fallback: str) -> str:
    ...
    digest = hashlib.sha1(stripped.encode("utf-8"), usedforsecurity=False).hexdigest()[
        :8
    ]  # nosemgrep
...
class PartnerManager:
    ...
    async def _outbound_router(self, partner_id: str, bus: Any, instance: PartnerInstance) -> None:
        ...
        try:
            ...
            from cognispheretutor.partners.bus.events import OutboundMessage as _OMsg
