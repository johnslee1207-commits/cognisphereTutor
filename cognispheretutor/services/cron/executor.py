...
from cognispheretutor.services.cron.service import CronJob
...
async def _maybe_send_desktop_notification(job: CronJob, text: str) -> None:
    ...
    cognisphereTutor still persists/delivers through its normal chat or partner
    ...
    title = f"cognisphereTutor: {job.name or 'Reminder'}"
...
async def _execute_chat_job(job: CronJob) -> tuple[str, str | None]:
    ...
    from cognispheretutor.multi_user.models import CurrentUser
    from cognispheretutor.multi_user.paths import local_admin_user, scope_for_user, user_context
