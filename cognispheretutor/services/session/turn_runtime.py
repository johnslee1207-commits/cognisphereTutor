...
from cognispheretutor.services.llm.utils import clean_thinking_tags
from cognispheretutor.services.path_service import get_path_service
from cognispheretutor.services.session.protocol import SessionStoreProtocol
...
def _llm_selection_dict(value: Any) -> dict[str, str] | None:
    from cognispheretutor.services.model_selection import LLMSelection
...
class TurnRuntimeManager:
    ...
    def __init__(self, store: SessionStoreProtocol | None = None) -> None:
        from cognispheretutor.services.session import get_session_store
    ...
    async def start_turn(self, payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        try:
            from cognispheretutor.runtime.request_contracts import validate_capability_config
        ...
        if llm_selection:
            try:
                from cognispheretutor.multi_user.model_access import apply_allowed_llm_selection
            ...
            from cognispheretutor.multi_user.model_access import (
        ...
        if llm_selection:
            ...
            from cognispheretutor.services.model_selection import (
                LLMSelection,
        ...
        if payload.get("tools") is None:
            try:
                from cognispheretutor.api.routers.settings import get_enabled_optional_tools
    ...
    async def _run_turn(self, execution: _TurnExecution) -> None:
        ...
        try:
            from cognispheretutor.agents.notebook import NotebookAnalysisAgent
            from cognispheretutor.book.context import build_book_context
            ...
            from cognispheretutor.services.model_selection.runtime import (
                activate_llm_selection,
            )
            from cognispheretutor.services.model_selection.runtime import (
                reset_llm_selection as reset_active_llm_selection,
            )
            from cognispheretutor.services.notebook import get_notebook_manager
            from cognispheretutor.services.session.context_builder import ContextBuilder
            ...
            from cognispheretutor.services.storage import get_attachment_store
            ...
            from cognispheretutor.utils.document_extractor import extract_documents_from_records
