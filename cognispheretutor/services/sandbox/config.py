...
* ``COGNISPHERETUTOR_SANDBOX_RUNNER_URL`` set ⇒ runner sidecar (Docker deployment);
* else on Linux with a functional ``bwrap`` ⇒ bwrap (bare-metal);
* else, only when ``COGNISPHERETUTOR_SANDBOX_ALLOW_SUBPROCESS=1`` ⇒ restricted
...
:mod:`cognispheretutor.tools.exec_tool`). Per-user quotas live in
:mod:`cognispheretutor.services.sandbox.quota`.
"""
...
RUNNER_URL_ENV = "COGNISPHERETUTOR_SANDBOX_RUNNER_URL"
ALLOW_SUBPROCESS_ENV = "COGNISPHERETUTOR_SANDBOX_ALLOW_SUBPROCESS"

# Per-user execution quotas (see quota.py). Conservative defaults; override
# via the matching env vars.
MAX_CONCURRENT_ENV = "COGNISPHERETUTOR_SANDBOX_MAX_CONCURRENT"
MAX_PER_MINUTE_ENV = "COGNISPHERETUTOR_SANDBOX_MAX_PER_MINUTE"
