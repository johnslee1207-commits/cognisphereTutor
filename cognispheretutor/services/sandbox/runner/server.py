...
(:class:`cognispheretutor.services.sandbox.backends.RunnerSidecarBackend`).
...
Wire contract (must match ``RunnerSidecarBackend``):
  ...
  ``POST /exec``   -> request/response JSON described by the dataclasses in
                      :mod:`cognispheretutor.services.sandbox.spec`. Request::
...
Mounts note:
  ...
  (``COGNISPHERETUTOR_RUNNER_ALLOWED_WORKDIRS``) as defence in depth.
"""
...
_ALLOWED_WORKDIR_ROOTS = [
    ...
    for root in os.environ.get(
        "COGNISPHERETUTOR_RUNNER_ALLOWED_WORKDIRS",
