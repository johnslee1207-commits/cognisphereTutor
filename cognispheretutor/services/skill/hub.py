...
A hub skill is the same artefact cognisphereTutor already speaks natively — a
...
# cognisphereTutor ships pointing at EduHub, its native open skill registry: a bare
# ``install <slug>`` with no ``<hub>:`` prefix resolves here.
DEFAULT_HUB = "eduhub"
_CLAWHUB_BASE_URL = "https://clawhub.ai/api/v1"
_EDUHUB_BASE_URL = "https://eduhub.cognispheretutor.info/api/v1"
...
class ClawHubProvider:
    ...
    def fetch(self, slug: str, *, version: str | None = None) -> FetchedSkill:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="cognispheretutor-skill-"))
        try:
    ...
    def catalog(
        ...
        cognisphereTutor-native browser renders without a second round-trip.
        """
        if query.strip():
