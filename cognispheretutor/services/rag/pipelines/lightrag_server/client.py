...
* ``POST /query`` with ``only_need_context=True`` — return the grounded context
  the server retrieved, WITHOUT its own generation. cognisphereTutor's chat loop keeps
...
def _sources_from_references(references: Any) -> list[dict[str, Any]]:
    """Map a LightRAG ``references`` list into cognisphereTutor's ``sources`` shape."""
