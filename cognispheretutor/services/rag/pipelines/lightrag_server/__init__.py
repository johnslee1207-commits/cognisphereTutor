...
standalone LightRAG server the user runs and indexed themselves. cognisphereTutor never
indexes or stores anything locally: retrieval is offloaded to the server's
``/query`` endpoint (context only — cognisphereTutor's chat LLM still writes the
