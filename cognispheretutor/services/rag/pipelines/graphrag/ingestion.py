...
(its optional ``markitdown`` reader is just a converter). So cognisphereTutor owns the
"document → text" step and hands GraphRAG ready ``.txt`` files. We deliberately
reuse cognisphereTutor's existing extraction primitives here so multimodal/parsing
stays a cognisphereTutor-side concern (matching how the LlamaIndex pipeline already
