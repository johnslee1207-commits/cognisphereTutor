...
Documents are turned into a MinerU-style ``content_list`` by cognisphereTutor's shared
parse layer (``cognispheretutor/services/parsing``) — the same bridge the question
    ...
    def _ensure_available(self) -> None:
        if not lr_config.is_lightrag_available():
            raise lr_config.LightRagNotAvailableError(
                ...
                "`pip install 'cognispheretutor[rag-lightrag]'` to use LightRAG knowledge bases."
            )
