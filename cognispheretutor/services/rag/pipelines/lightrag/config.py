"""Bridge cognisphereTutor's runtime config into LightRAG / RAG-Anything.
...
builds the three adapters LightRAG needs from cognisphereTutor's already-resolved LLM /
...
Decoupling notes:
* ``llm_model_func`` / ``vision_model_func`` wrap cognisphereTutor's unified model
  callables and DROP LightRAG's internal kwargs (``hashing_kv``,
  ``keyword_extraction``, …) so they never leak into ``factory.complete``.
* ``embedding_func`` reuses cognisphereTutor's embedding client, wrapped in LightRAG's
...
class LightRagNotConfiguredError(RuntimeError):
    """Raised when cognisphereTutor's LLM / embedding config can't back LightRAG."""


def is_lightrag_available() -> bool:
    """True when RAG-Anything (which bundles LightRAG) can be imported.

    Opt-in extra: ``pip install 'cognispheretutor[rag-lightrag]'``. Until installed the
...
def query_kwargs_from_settings() -> dict:
    ...
    """
    try:
        from cognispheretutor.services.config import load_lightrag_settings
...
def build_llm_model_func():
    """Wrap cognisphereTutor's unified LLM callable for LightRAG.
...
def build_vision_model_func():
    """Wrap cognisphereTutor's vision-capable callable for RAG-Anything's image step."""
    from cognispheretutor.services.llm import get_llm_client
...
def build_embedding_func():
    """Wrap cognisphereTutor's embedding client in LightRAG's ``EmbeddingFunc``."""
    from lightrag.utils import EmbeddingFunc

    from cognispheretutor.services.embedding import get_embedding_client, get_embedding_config
