...
fix. All imports are lazy so cognisphereTutor runs fine without the optional dependency
installed.

A RAG-Anything instance is built from cognisphereTutor's LLM/vision/embedding adapters
...
def build_rag(working_dir: Path) -> Any:
    ...
    # cognisphereTutor always feeds RAG-Anything a pre-parsed ``content_list`` (the
    # parse layer runs upstream via cognisphereTutor's own ParseService), so
