...
* ``graphrag``            — local knowledge-graph retrieval (microsoft/graphrag);
                            optional dependency, ``pip install 'cognispheretutor[graphrag]'``.
* ``lightrag``            — graph + vector retrieval (HKUDS/LightRAG, multimodal
                            via RAG-Anything); optional dependency,
                            ``pip install 'cognispheretutor[rag-lightrag]'``.
...
def provider_uses_embedding_versions(provider: Optional[str]) -> bool:
    ...
    Today only the LlamaIndex pipeline uses cognisphereTutor's active embedding
...
def list_pipelines() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "name": "GraphRAG",
            "description": "Local knowledge-graph retrieval (global/local/drift/basic). Needs `pip install 'cognispheretutor[graphrag]'`; indexing is LLM-heavy.",
        ...
        },
        {
            ...
            "name": "LightRAG",
            "description": "Graph + vector retrieval with multimodal parsing (naive/local/global/hybrid/mix). Needs `pip install 'cognispheretutor[rag-lightrag]'`; indexing is LLM-heavy.",
