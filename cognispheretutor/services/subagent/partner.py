...
*partner session key*. The first consult of a cognisphereTutor chat session has none,
so we mint a fresh ``dt-…`` key and return it; the cross-turn registry
(:mod:`cognispheretutor.services.subagent.sessions`) remembers it against
(chat session, connection), so every later consult in the same cognisphereTutor chat —
within one turn or across turns — resumes the SAME partner session. The partner
page then sees one complete history session per cognisphereTutor chat, titled from the
...
from cognispheretutor.services.subagent.base import OnEvent, SubagentBackend
from cognispheretutor.services.subagent.config import BackendConfig
from cognispheretutor.services.subagent.types import (
    EVENT_ERROR,
    EVENT_REASONING,
    EVENT_TEXT,
    EVENT_TOOL,
...
if TYPE_CHECKING:  # avoid importing the core/partner packages at module load
    from cognispheretutor.core.stream import StreamEvent
...
class PartnerBackend(SubagentBackend):
    ...
    async def consult(
        ...
        *,
        ...
        # a cognisphereTutor chat → mint a stable, colon-free key; the registry threads
