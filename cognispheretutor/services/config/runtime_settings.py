...
from cognispheretutor.services.file_io import atomic_write_json as _atomic_write_json
...
_DEFAULT_MINERU_ENGINE: dict[str, Any] = {
    ...
    # separate conda) so its heavy deps never conflict with cognisphereTutor's.
    "local_cli_path": "",
...
# cognisphereTutor's VLM to describe images.
...
# cognisphereTutor's legacy text extractors for PDF / Office / text-like files.
...
IGNORE_PROCESS_OVERRIDES_ENV = "COGNISPHERETUTOR_IGNORE_PROCESS_ENV_OVERRIDES"
...
class RuntimeSettingsService:
    ...
    def render_environment(self) -> dict[str, str]:
        ...
        return {
            ...
            # rewrites /api/* and /ws/* to COGNISPHERETUTOR_API_BASE_URL and uses
            # COGNISPHERETUTOR_AUTH_ENABLED to gate the login redirect. The launcher and
            # the Docker entrypoint both export these through render_environment,
            # so the two deployment paths stay in sync. COGNISPHERETUTOR_API_BASE_URL is
            ...
            "COGNISPHERETUTOR_AUTH_ENABLED": _bool_env(auth["enabled"]),
    ...
    def _apply_system_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        if value := self._process_env_value("COGNISPHERETUTOR_SANDBOX_ALLOW_SUBPROCESS"):
    ...
    def _apply_llamaindex_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        # (COGNISPHERETUTOR_RAG_RETRIEVAL_PROFILE / RAG_RETRIEVAL_PROFILE); preserve it.
        payload = dict(settings)
        if value := (
            self._process_env_value("COGNISPHERETUTOR_RAG_RETRIEVAL_PROFILE")
