...
class ConfigTestRunner:
    ...
    async def _test_llm(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from cognispheretutor.services.llm import clear_llm_config_cache, get_token_limit_kwargs
        from cognispheretutor.services.llm import complete as llm_complete
    ...
    async def _test_embedding(
        ...
        from cognispheretutor.services.embedding.client import EmbeddingClient
        from cognispheretutor.services.embedding.config import EmbeddingConfig
        ...
        probe_texts = [
            "cognisphereTutor embedding smoke test",
            "cognisphereTutor retrieval batch probe",
        ]
    ...
    def _test_search(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from cognispheretutor.services.search import web_search
        ...
        run.emit("info", "Running search query: cognisphereTutor configuration health check")
        result = web_search("cognisphereTutor configuration health check", provider=provider)
        run.emit(
            "response",
    ...
    async def _test_tts(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from cognispheretutor.services.config.provider_runtime import resolve_tts_runtime_config
        from cognispheretutor.services.voice import synthesize_speech
        ...
        sample = "cognisphereTutor voice check. 这是一段语音合成测试。"
    ...
    async def _test_stt(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from cognispheretutor.services.config.provider_runtime import resolve_stt_runtime_config
        from cognispheretutor.services.voice import transcribe_audio
