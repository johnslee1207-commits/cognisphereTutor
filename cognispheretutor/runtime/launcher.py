"""Local Web launcher for the installed cognisphereTutor app."""
...
from cognispheretutor.runtime.home import COGNISPHERETUTOR_HOME_ENV, PACKAGE_ROOT, get_runtime_home
from cognispheretutor.runtime.openmaic_sidecar import plan_openmaic_sidecar
...
def _reset_runtime_singletons() -> None:
    """Make a just-selected COGNISPHERETUTOR_HOME visible to path/config singletons."""
    try:
...
def _packaged_web_dir() -> Path | None:
    try:
        import cognispheretutor_web
    except ImportError:
        return None
    path = Path(cognispheretutor_web.__file__).resolve().parent
...
def _copy_packaged_web_if_needed(
    ...
    *,
    ...
    marker = cache / ".cognispheretutor-web-runtime.json"
...
def _resolve_frontend(
    ...
    *,
    ...
    raise SystemExit(
        "cognisphereTutor Web assets are not installed. Install the full app with `pip install -U cognispheretutor`, "
...
def start(home: str | Path | None = None) -> None:
    ...
    os.environ[COGNISPHERETUTOR_HOME_ENV] = str(runtime_home)
    ...
    common_env[COGNISPHERETUTOR_HOME_ENV] = str(runtime_home)
    ...
    common_env["COGNISPHERETUTOR_API_BASE_URL"] = backend_url
    common_env["COGNISPHERETUTOR_AUTH_ENABLED"] = "true" if auth_enabled else "false"
    common_env["PYTHONUNBUFFERED"] = "1"
    common_env["PYTHONIOENCODING"] = "utf-8:replace"

    openmaic_plan = plan_openmaic_sidecar(
        integrations,
        runtime_home=runtime_home,
        process_env=common_env,
    )
    if openmaic_plan.should_start:
        ...
        _log(f"{'OpenMAIC':<10} {openmaic_plan.origin} (managed)")
    elif openmaic_plan.reason:
        _log(f"{'OpenMAIC':<10} {openmaic_plan.reason}")
    ...
    backend_cmd = [
        ...
        # middleware (cognispheretutor/api/main.py) surfaces only non-200s, so routine
    ...
    openmaic: ManagedProcess | None = None
    ...
    try:
        if openmaic_plan.should_start:
            _log("Starting OpenMAIC runtime ...")
            sidecar_env = common_env.copy()
            sidecar_port = urlparse.urlparse(openmaic_plan.origin).port
            if sidecar_port is not None:
                sidecar_env["PORT"] = str(sidecar_port)
                sidecar_env["OPENMAIC_COURSE_RUNTIME_PORT"] = str(sidecar_port)
            openmaic = _spawn(
                openmaic_plan.command,
                cwd=openmaic_plan.cwd,
                env=sidecar_env,
                name="openmaic",
            )
            processes.append(openmaic)
            _wait_for_http(
                name="OpenMAIC",
                url=openmaic_plan.health_url,
                process=openmaic,
