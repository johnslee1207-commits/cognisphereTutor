"""Branded banner + localized labels for ``cognispheretutor start`` / ``cognispheretutor init``.
...
from cognispheretutor.__version__ import __version__
...
LABELS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "init.next_step": "Run `cognispheretutor start` to launch cognisphereTutor.",
        "init.choice": "Choice",
        ...
        "start.frontend_restart_failed": (
            ...
            "Stop PID {pid} and run `cognispheretutor start` again."
        ),
        ...
        "start.port_in_use": (
            "cognisphereTutor cannot start because port(s) already in use: {ports}. "
    ...
    "zh": {
        ...
        "init.next_step": "运行 `cognispheretutor start` 启动 cognisphereTutor。",
        "init.choice": "请选择",
        ...
        "start.frontend_restart_failed": (
            ...
            "请先停止 PID {pid},然后重新运行 `cognispheretutor start`。"
        ),
        ...
        "start.port_in_use": (
            "无法启动 cognisphereTutor,端口已被占用: {ports}。"
...
def render_banner(language: str | None, *, mode_key: str | None = None) -> Panel:
    ...
    return Panel(
        Align.left(body),
        title="[bold bright_cyan]cognisphereTutor[/]",
