...
Controls whether cognisphereTutor is running as a CLI application or an API server.
...
def _resolve_mode() -> RunMode:
    raw = os.environ.get("COGNISPHERETUTOR_MODE", "").strip().lower()
...
def set_mode(mode: RunMode) -> None:
    ...
    global _current_mode
    _current_mode = mode
    os.environ["COGNISPHERETUTOR_MODE"] = mode.value
