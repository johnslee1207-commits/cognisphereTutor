...
The main entry point is :class:`~cognispheretutor.agents.question.pipeline.QuestionPipeline`.
...
def __getattr__(name: str) -> Any:
    if name == "AgentCoordinator":
        module = import_module("cognispheretutor.agents.question.coordinator")
        return getattr(module, name)
    if name == "FollowupAgent":
        module = import_module("cognispheretutor.agents.question.agents.followup_agent")
        return getattr(module, name)
    if name in {"QuestionPipeline", "QuizTemplate", "QuizPair", "QuizPlan", "QuizHistoryEntry"}:
        module = import_module("cognispheretutor.agents.question.pipeline")
        return getattr(module, name)
