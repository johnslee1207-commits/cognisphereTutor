...
from cognispheretutor.i18n import StatusI18n
from cognispheretutor.runtime.request_contracts import get_capability_request_schema
...
class DeepQuestionCapability(BaseCapability):
    ...
    async def run(self, context: UnifiedContext, stream: StreamBus) -> None:
        ...
        if mode != "mimic":
            ...
            from cognispheretutor.agents.question.history import load_session_quiz_history
            from cognispheretutor.agents.question.pipeline import QuestionPipeline
            from cognispheretutor.agents.question.request_config import (
                build_question_runtime_config,
            )
            from cognispheretutor.services.config import load_config_with_main
    ...
    async def _run_mimic_mode(
        ...
        * ``[Attached Documents]`` in  → no paper available; fall back to
        ...
        from cognispheretutor.services.config import load_config_with_main
        from cognispheretutor.services.parsing.engines.mineru.config import MinerUError
