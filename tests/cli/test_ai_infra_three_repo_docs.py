...
def test_ai_infra_three_repo_docs_define_content_only_and_full_twin_modes() -> None:
    doc = (ROOT / "docs" / "AI_INFRA_THREE_REPO_DEPLOYMENT_ZH.md").read_text(encoding="utf-8")
...
def test_ai_infra_env_example_points_to_plugin_and_twin_boundaries() -> None:
    example = (ROOT / "examples" / "ai_infra_three_repo.env.example").read_text(encoding="utf-8")
