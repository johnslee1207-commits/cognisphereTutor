...
function getTraceHeader(
  ...
  if (
    ...
    // top-level status row carries the verbose "cognisphereTutor Reflecting…"
...
function detectStreamingMode(
  ...
  for (let idx = events.length - 1; idx >= 0; idx -= 1) {
    ...
    if (event.type === "content" && callKind === "llm_final_response") {
      ...
      // still open; keep the top-level title on "cognisphereTutor Exploring…" until
      // the bus moves on.
      if (event.stage === "exploring") return "exploring";
...
export function StreamingStatus({
  ...
  const name = agentName?.trim() || "cognisphereTutor";
  ...
  else if (mode === "responded") modeLabel = t("cognisphereTutor responded.");
 ...
 * reasoning trace once cognisphereTutor stops working and starts (or has finished)
 * its answer.
 *
 ...
 * ("cognisphereTutor Exploring… · 8s", settling to "cognisphereTutor responded. · 10s")
 * with the exploring trace nested directly beneath it.
 *
 * The trace is expanded by default while cognisphereTutor is still reasoning /
