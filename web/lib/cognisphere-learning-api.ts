...
export function masteryChatHref(
  ...
  if (opts?.focusModule) {
    params.set("focus_module", opts.focusModule);
  }
  if (opts?.focusObjective) {
    params.set("focus_objective", opts.focusObjective);
  }
  if (opts?.launch) {
    params.set("launch", opts.launch);
  }
