...
``cognispheretutor skills login`` opens the hub's GitHub/Google authorization page in
...
def hub_origin_from_base(base_url: str) -> str:
    ...
    ``https://eduhub.cognispheretutor.info/api/v1`` -> ``https://eduhub.cognispheretutor.info``.
    """
    b = base_url.rstrip("/")
