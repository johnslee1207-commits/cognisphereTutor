"""CLI entry point for the standalone ``cognispheretutor-cli`` package."""
...
from cognispheretutor.runtime.mode import RunMode, set_mode

from .book import register as register_book
from .chat import register as register_chat
from .cognisphere_cmd import register as register_cognisphere
...
app = typer.Typer(
    ...
    help="cognisphereTutor CLI – agent-first interface for capabilities, tools, and knowledge.",
...
cognisphere_app = typer.Typer(
    help="Cognisphere Learning Plugins: discover, negotiate, validate, import, compose (DT-P1…P6)."
)
...
app.add_typer(skill_app, name="skills")  # alias: `cognispheretutor skills …`
...
app.add_typer(cognisphere_app, name="cognisphere")
...
register_cognisphere(cognisphere_app)
register_init(app)
...
def run_capability(
    ...
    from cognispheretutor.app import cognisphereTutorApp
    ...
    maybe_run(run_turn_and_render(app=cognisphereTutorApp(), request=request, fmt=fmt))
...
def start(
    ...
    from cognispheretutor.runtime.launcher import start as start_web
...
def serve(
    ...
    """Start the cognisphereTutor API server."""
    import asyncio
    import sys

    set_mode(RunMode.SERVER)
    if port is None:
        from cognispheretutor.services.setup import get_backend_port
    ...
    try:
        ...
        console.print(
            ...
            "Run: pip install -U cognispheretutor"
        )
        raise typer.Exit(code=1)

    from cognispheretutor.services.config import get_ws_max_size
