...
# lives at module scope so a single ``cognispheretutor chat`` session shares one
# ring across turns; ``cognispheretutor run`` doesn't read it (single-shot mode),
...
async def stream_turn_as_json(*, app: cognisphereTutorApp, turn_id: str) -> None:
...
async def render_turn_stream(*, app: cognisphereTutorApp, turn_id: str) -> str:
...
class TurnStreamRenderer:
    ...
    async def _on_wait_for_input(self, item: dict[str, Any]) -> None:
        ...
        from cognispheretutor.core.stream_bus import get_bus
