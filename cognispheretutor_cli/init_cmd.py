...
``cognispheretutor init`` walks the user through a four-step wizard (ports → LLM →
...
review panel) lives in :mod:`cognispheretutor_cli.init_wizard`. This module is
...
from cognispheretutor.runtime.home import COGNISPHERETUTOR_HOME_ENV, get_runtime_home

from . import init_wizard as wiz


def _reset_runtime_singletons() -> None:
    """Drop cached service instances so the new COGNISPHERETUTOR_HOME takes effect.

    ``cognispheretutor init`` may pass ``--home`` to target a different workspace; the
...
def _embedding_step(
    ...
    from cognispheretutor.services.config.embedding_endpoint import (
