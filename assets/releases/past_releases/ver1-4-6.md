# cognisphereTutor v1.4.6 Release Notes
...
- **Login** accepts a username (not just an email), surfaces bad-login errors
  ...
  flag.
- **CLI** gains `cognispheretutor skill` commands — browser login plus publish/update
  for your own skills.
...
- Drop-in from v1.4.5: `pip install -U cognispheretutor`; Docker users pull
  `ghcr.io/hkuds/cognispheretutor:latest`. No migrations — existing data loads as-is.
...
- New engines and services are opt-in: GraphRAG needs the extra
  (`pip install 'cognispheretutor[graphrag]'`), document parsing needs
  `cognispheretutor[parse]`, and voice / image / video need their model catalogs
  configured.

**Full Changelog**: https://github.com/HKUDS/cognisphereTutor/compare/v1.4.5...v1.4.6
