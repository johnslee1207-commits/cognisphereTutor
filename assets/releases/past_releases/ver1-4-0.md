# cognisphereTutor v1.4.0 Release Notes
...
isolation hardening, and the `cognispheretutor start` / `cognispheretutor init` launcher)
...
- **`build_provider_extra_kwargs`** in `cognispheretutor/core/agentic/client.py`
...
The `cognispheretutor chat` REPL surface was tightened so it matches the public
docs at [cognispheretutor.info](https://cognispheretutor.info).

- `cognispheretutor chat --config key=value` and `--config-json '<json>'` seed
...
- `cognispheretutor plugin info <name>` now also returns `cli_aliases` and the
...
- `.gitignore` covers `.playwright-cli/`; stale `.playwright-cli/`
  captures, `.env.example_CN`, and `cognisphereTutor.code-workspace` removed.
...
- New `tests/cli/test_docs_contract.py` keeps the public site docs in
  sync with the CLI surface (links + `cognispheretutor …` examples).
- `tests/scripts/test_docker_compose.py` + `web/tests/api-resolve-base.test.ts`
  cover the new placeholder detection.

## Upgrade Notes
- **From v1.4.0-beta:** drop-in. PyPI installs with `pip install -U cognispheretutor`
...
**Full Changelog**: https://github.com/HKUDS/cognisphereTutor/compare/v1.4.0-beta...v1.4.0
