# cognisphereTutor v1.4.4 Release Notes
...
cognisphereTutor skills share their format with the Agent-Skills ecosystem
(`SKILL.md` + support files), so external registries are now a native
source for the skill library:

```bash
cognispheretutor skill search "git release notes"        # semantic search on the hub
cognispheretutor skill install clawhub:gh-release-notes  # verify → fetch → register
cognispheretutor skill install some-skill@1.2.0          # hub defaults to clawhub; @ pins a version
cognispheretutor skill list                              # local skills with hub provenance
```
...
- Drop-in from v1.4.3: `pip install -U cognispheretutor`; Docker users pull
  `ghcr.io/hkuds/cognispheretutor:latest`. No migrations.
- `cognispheretutor skill install` talks to clawhub.ai over HTTPS; air-gapped
  deployments can point a mirror or a fetch CLI at it via
  `settings/skill_hubs.json`.

**Full Changelog**: https://github.com/HKUDS/cognisphereTutor/compare/v1.4.3...v1.4.4
