...
Learning Space：输入自然语言目标 → **推荐插件** / 一键 **组合导入**；或手动多选域 → **组合并导入**；或单域 **导入**。侧栏 **AWS Digital Twin Mastery** 可跑离线练习、执行 Handshake + Learning→Twin，并导入 `aws_certification` 进入 Mastery Path 对话；AWS 路径地图内同样可触发。点击「跑离线练习」后，右侧面板展示场景、服务对比、反馈与辅导回合等学习内容（步骤 `learner` 字段）。Twin 包（`kind=twin`）不进入课程库 import 列表。Cognisphere 路径展示 `suggest-focus` / `plan-path` 的焦点与技能路径提示。路径详情展示**能力雷达**（模块轴 + 薄弱知识点），数据来自 Mastery 地图。
