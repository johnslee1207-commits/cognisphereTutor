...
# cognisphereTutor : Tutorat Personnalisé à Vie
...
[Fonctionnalités](#-fonctionnalités-clés) · [Démarrage](#-démarrage) · [Explorer](#-explorer-cognispheretutor) · [CLI](#%EF%B8%8F-cognispheretutor-cli--interface-native-à-lagent) · [Écosystème](#-écosystème--eduhub--la-communauté-de-compétences) · [Communauté](#-communauté)

</div>

---

> 🤝 **Nous accueillons toutes les formes de contribution !** Votez sur les éléments de la feuille de route ou proposez-en de nouveaux sur [`Roadmap`](https://github.com/HKUDS/cognisphereTutor/issues/498), et consultez notre [Guide de contribution](CONTRIBUTING.md) pour la stratégie de branches, les normes de code et comment démarrer.

### 📰 Actualités

- **2026-05-22** 🌐 Site de documentation officiel en ligne sur [**cognispheretutor.info**](https://cognispheretutor.info/) — guides, références et tours des capacités en un seul endroit.
...
- **2026-04-10** 📄 Notre article est en ligne sur arXiv — lisez le [preprint](https://arxiv.org/abs/2604.26962) pour la conception et les idées derrière cognisphereTutor.
- **2026-02-06** 🚀 10 000 étoiles en seulement 39 jours ! Un immense merci à notre incroyable communauté.
- **2026-01-01** 🎊 Bonne Année ! Rejoignez notre [Discord](https://discord.gg/eRsjPgMU4t), [WeChat](https://github.com/HKUDS/cognisphereTutor/issues/78), ou les [Discussions](https://github.com/HKUDS/cognisphereTutor/discussions) — façonnons cognisphereTutor ensemble.
- **2025-12-29** 🎓 cognisphereTutor est officiellement lancé !

## ✨ Fonctionnalités Clés

cognisphereTutor est un espace de travail d'apprentissage natif à l'agent qui connecte le tutorat, la résolution de problèmes, la génération de quiz, la recherche, la visualisation et la pratique de maîtrise dans un système extensible.
...
cognisphereTutor propose quatre chemins d'installation. Ils partagent tous une même structure d'espace de travail : les paramètres résident dans `data/user/settings/` sous le répertoire depuis lequel vous lancez l'application (ou sous `COGNISPHERETUTOR_HOME` / `cognispheretutor start --home` si vous en définissez un explicitement). Pour l'application complète, le flux recommandé est **choisir un répertoire d'espace de travail → installer → `cognispheretutor init` → `cognispheretutor start`**.

<details>
<summary><b>Option 1 — Installer depuis PyPI</b> · application Web locale complète + CLI, sans clonage</summary>

Application Web locale complète + CLI, sans clonage requis. Nécessite **Python 3.11+** et un runtime **Node.js 20+** dans le PATH (le serveur standalone Next.js packagé est lancé par `cognispheretutor start`).

```bash
mkdir -p my-cognispheretutor && cd my-cognispheretutor
pip install -U cognispheretutor
cognispheretutor init     # invite pour les ports + fournisseur LLM + embedding optionnel
cognispheretutor start    # démarre le backend + frontend ; gardez le terminal ouvert
```

`cognispheretutor init` demande le port backend (par défaut `8001`), le port frontend (par défaut `3782`), le fournisseur LLM / URL de base / clé API / modèle, et un fournisseur d'embedding optionnel pour la Base de Connaissances / RAG.

Après `cognispheretutor start`, ouvrez l'URL frontend affichée dans le terminal — par défaut [http://127.0.0.1:3782](http://127.0.0.1:3782). Appuyez sur `Ctrl+C` dans ce terminal pour arrêter le backend et le frontend. Ignorer `cognispheretutor init` est possible pour un essai rapide ; l'application démarre avec les ports par défaut et des paramètres de modèle vides, à configurer plus tard dans **Paramètres → Modèles**.
...
**Serveur de développement bloqué :** si `cognispheretutor start` signale un frontend existant qui ne répond pas, arrêtez le PID qu'il affiche. Si aucun processus Next.js ne tourne réellement, les fichiers de verrou sont périmés — supprimez-les et réessayez :
...
- `ghcr.io/hkuds/cognispheretutor:latest` — version stable
- `ghcr.io/hkuds/cognispheretutor:pre` — pré-version, si disponible
...
- **Détaché :** ajoutez `-d`, puis `docker logs -f cognispheretutor` pour suivre, `docker stop cognispheretutor` pour arrêter, `docker rm cognispheretutor` avant de réutiliser le nom. Le volume `cognispheretutor-data` conserve vos paramètres et votre espace de travail à travers les redémarrages.
...
l'authentification désactivée, cognisphereTutor autorise les origines de navigateur HTTP/HTTPS normales par défaut.
...
`cognispheretutor init --cli` partage la même structure `data/user/settings/` que l'application complète mais ignore les invites de ports backend/frontend et désactive les embeddings par défaut (choisissez `Yes` si vous prévoyez d'utiliser `cognispheretutor kb …` ou les outils RAG). Il écrit quand même une structure d'exécution complète (`system.json`, `auth.json`, `integrations.json`, `model_catalog.json`, `main.yaml`, `agents.yaml`) et demande quand même le fournisseur LLM actif et le modèle.

<details>
<summary><b>Commandes courantes</b></summary>

```bash
cognispheretutor chat                                          # REPL interactif
...
L'installation locale `cognispheretutor-cli` n'inclut pas de ressources Web ni de dépendances serveur. Conservez le checkout des sources — l'installation modifiable y pointe. Pour ajouter l'application Web plus tard, installez le paquet PyPI (Option 1) et exécutez `cognispheretutor init` + `cognispheretutor start` depuis le même espace de travail.
...
- **docker-compose :** acheminé à la place vers un **sidecar runner** durci et
  à moindres privilèges (`Dockerfile.runner`) via `COGNISPHERETUTOR_SANDBOX_RUNNER_URL` —
...
`COGNISPHERETUTOR_SANDBOX_ALLOW_SUBPROCESS=0`) pour désactiver l'exécution côté hôte, au
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/OVERVIEW.png" alt="Accueil cognisphereTutor — l'espace de travail Chat avec chaque surface dans la barre latérale" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/system%20architecture.png" alt="Architecture du système cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/home/00-overview.png" alt="Espace de travail Chat de cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/chat-agent-loop.png" alt="Boucle d'agent Chat de cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/partners/00-partners%20overview.png" alt="Espace de travail Partners de cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/partners-architecture.png" alt="Architecture Partners de cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/myagents/00-overview.png" alt="Espace de travail My Agents de cognisphereTutor" width="900">
</div>

My Agents transforme d'autres agents en contexte pour cognisphereTutor, et fait deux choses distinctes. **Connectez un agent en direct** — un Claude Code ou Codex CLI sur votre machine, ou l'un de vos Partners — et consultez-le depuis l'intérieur d'un tour de chat : cognisphereTutor *exécute* réellement l'autre agent et diffuse son travail dans le panneau d'Activité via l'outil `consult_subagent`. Sélectionnez-le avec la puce Agent (ou tapez `@`), et définissez combien de rounds la consultation peut prendre.
...
**Importez des conversations passées** — apportez votre historique Claude Code et Codex existant comme des agents nommés, consultables et reprenables. Choisissez les jours à importer ; l'actualisation les re-synchronise. Référencez une conversation importée depuis n'importe quel tour de chat via `+` → My Agents, et cognisphereTutor la lit comme une transcription tierce — elle reste *leur* conversation, pas la voix propre de cognisphereTutor.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/co-writer/00-overview.png" alt="Espace de travail Co-Writer de cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/book/00-book_overview.png" alt="Bibliothèque de livres cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/knowledge/00-overview.png" alt="Knowledge Center de cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/learning-space/00-overview.png" alt="Hub Learning Space de cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/00-overview.png" alt="Aperçu de la mémoire cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/01-3%20layer%20memory%20graph.png" alt="Graphe de mémoire cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/settings/00-setting%20overview.png" alt="Hub Settings de cognisphereTutor" width="900">
</div>
...
L'authentification est **désactivée par défaut** — cognisphereTutor fonctionne en mode mono-utilisateur. Activez-la et un seul arbre `data/` héberge un espace de travail admin, des espaces de travail per-utilisateur isolés et des espaces de travail partner côte à côte :
...
**Activer :** activez l'auth dans `data/user/settings/auth.json`, redémarrez `cognispheretutor start`, enregistrez le premier admin sur `/register`, puis ajoutez des utilisateurs depuis `/admin/users` et assignez des modèles, KB, compétences, Partners, politique d'outils/MCP et accès à l'exécution de code via des attributions.
...
## ⌨️ cognisphereTutor CLI — Interface Native à l'Agent

Un seul binaire `cognispheretutor`, deux façons d'accéder : un **REPL** interactif pour ceux qui vivent dans le terminal, et du **JSON** structuré pour d'autres agents qui pilotent cognisphereTutor comme un outil. Les mêmes capacités, outils et bases de connaissances dans les deux cas.

<details>
<summary><b>Piloter vous-même</b></summary>

`cognispheretutor chat` ouvre un REPL interactif ; `cognispheretutor run <capacité> "<message>"` exécute un seul tour et quitte. Les deux acceptent les mêmes drapeaux `--capability`, `--tool`, `--kb` et `--config`.
...
Le dépôt inclut un [`SKILL.md`](SKILL.md) racine — un document de passation de ~150 lignes qui enseigne à tout LLM utilisant des outils la totalité de la surface en une seule lecture. Remettez-le à Claude Code, Codex ou OpenCode (ils récupèrent `SKILL.md` automatiquement), ou enveloppez `cognispheretutor run` comme un outil dans une boucle LangChain / AutoGen. Recettes complètes : [Agent Handoff](https://cognispheretutor.info/docs/cli/agent-handoff/).
...
|:---|:---|
| `cognispheretutor init` | Créer ou mettre à jour `data/user/settings` pour l'espace de travail actuel |
| `cognispheretutor start [--home PATH]` | Lancer le backend + frontend ensemble |
| `cognispheretutor serve [--port PORT]` | Démarrer uniquement le backend FastAPI |
| `cognispheretutor run <capacité> <message>` | Exécuter un seul tour de capacité (`chat`, `deep_solve`, `deep_question`, `deep_research`, `visualize`, `math_animator`, `mastery_path`) ; ajoutez `--format json` pour la sortie NDJSON |
| `cognispheretutor chat` | REPL interactif avec contrôles de capacité, outil, KB, carnet et historique |
| `cognispheretutor partner list/create/start/stop` | Gérer les partners connectés à IM |
| `cognispheretutor kb list/info/create/add/search/set-default/delete` | Gérer les bases de connaissances LlamaIndex |
| `cognispheretutor skill search/install/list/remove/login/logout/publish/update` | Gérer les compétences, installer depuis les hubs et publier les siennes (`eduhub:<slug>` par défaut, voir Écosystème) |
| `cognispheretutor memory show/clear` | Inspecter les documents de mémoire L2/L3 ou effacer la mémoire L1/toute |
| `cognispheretutor session list/show/open/rename/delete` | Gérer les sessions partagées |
| `cognispheretutor notebook list/create/show/add-md/replace-md/remove-record` | Gérer les carnets depuis des fichiers Markdown |
| `cognispheretutor book list/health/refresh-fingerprints` | Inspecter les livres et actualiser les empreintes des sources |
| `cognispheretutor plugin list/info` | Inspecter les outils et capacités enregistrés |
| `cognispheretutor config show` | Afficher le résumé de configuration |
| `cognispheretutor provider login <fournisseur>` | Auth du fournisseur (OAuth login `openai-codex` ; `github-copilot` valide une session auth Copilot existante) |
...
Le paquet CLI uniquement se trouve dans `packaging/cognispheretutor-cli`. Dans ce checkout, installez-le depuis les sources :
...
Les compétences cognisphereTutor utilisent le format ouvert **Agent-Skills** — un dossier avec un livret de jeu `SKILL.md` (frontmatter YAML + Markdown) et des fichiers de référence optionnels. Rien dans ce format n'est spécifique à cognisphereTutor, donc tout registre qui parle le format devient une source pour votre bibliothèque. cognisphereTutor inclut **[EduHub](https://eduhub.cognispheretutor.info/)** — notre propre registre de compétences axé sur l'éducation — configuré comme hub par défaut.

<details>
<summary><b>EduHub — l'écosystème de compétences de cognisphereTutor</b></summary>

[**EduHub**](https://eduhub.cognispheretutor.info/) est le hub communautaire que cognisphereTutor a lancé pour partager des compétences d'agent orientées enseignement — tuteurs socratiques, constructeurs de fiches, retours sur les essais, plans d'examen, explicateurs de concepts, et plus encore. Il est intégré à cognisphereTutor, il n'y a donc rien à configurer : un slug nu ou un préfixe `eduhub:` le résout.
...
```bash
cognispheretutor skill search "socratic tutor"               # rechercher sur EduHub (le hub par défaut)
cognispheretutor skill install socratic-tutor                # récupérer → vérifier → enregistrer
cognispheretutor skill install eduhub:socratic-tutor@1.2.0   # épingler un hub et une version
cognispheretutor skill list                                  # compétences locales avec leur provenance de hub
```

**Publiez la vôtre** — emballez un `SKILL.md` et partagez-le avec la communauté :

```bash
cognispheretutor skill login                                 # connexion navigateur à EduHub
cognispheretutor skill publish ./my-skill                    # interactif : choisir une piste + étiquettes, puis uploader
cognispheretutor skill update                                # revenir en arrière ou publier une nouvelle version
```

EduHub est également un registre autonome compatible ClawHub, de sorte que les agents qui ne sont pas cognisphereTutor (Claude Code, Codex, …) peuvent l'utiliser directement via le CLI `eduhub` — `npx eduhub install socratic-tutor`.
...
- le frontmatter est normalisé selon le schéma de cognisphereTutor et `always:` est **supprimé**, donc une compétence téléchargée ne peut jamais se forcer dans chaque prompt système ;
...
Parce que cognisphereTutor parle le format ouvert Agent-Skills, **[ClawHub](https://clawhub.ai/)** fonctionne aussi comme une source de première classe — il est intégré aux côtés d'EduHub. Choisissez-le avec le préfixe hub :
...
| [**Claude Code**](https://github.com/anthropics/claude-code) | CLI de codage agentique qui a inspiré la boucle d'agent cognisphereTutor |
...
Nous voulons que cognisphereTutor continue d'itérer et de s'améliorer — et finalement de devenir un cadeau que nous offrons en retour à la communauté open-source. Notre [**feuille de route**](https://github.com/HKUDS/cognisphereTutor/issues/498) est mise à jour en continu ; votez sur les éléments ou proposez-en de nouveaux. Si vous souhaitez contribuer, consultez le [**Guide de contribution**](CONTRIBUTING.md) pour la stratégie de branches, les normes de code et comment démarrer.

<div align="center">

Nous espérons que cognisphereTutor deviendra un cadeau pour la communauté. 🎁

<a href="https://github.com/HKUDS/cognisphereTutor/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=HKUDS/cognisphereTutor&max=999" alt="Contributeurs" />
</a>
...
<a href="https://www.star-history.com/#HKUDS/cognisphereTutor&type=timeline&legend=top-left">
  <picture>
    ...
    <img alt="Graphique d'historique des étoiles" src="https://api.star-history.com/svg?repos=HKUDS/cognisphereTutor&type=timeline&legend=top-left" />
  </picture>
</a>
...
<p align="center">
 <a href="https://www.star-history.com/hkuds/cognispheretutor">
  <picture>
   ...
   <img alt="Classement Historique des Étoiles" src="https://api.star-history.com/badge?repo=HKUDS/cognisphereTutor" />
  </picture>
 </a>
</p>
...
<p>
  <img src="https://visitor-badge.laobi.icu/badge?page_id=HKUDS.cognisphereTutor&style=for-the-badge&color=00d4ff" alt="Vues">
</p>
