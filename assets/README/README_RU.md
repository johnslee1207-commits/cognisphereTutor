...
# cognisphereTutor: Персонализированное Обучение на Базе Агентов
...
[![WeChat](https://img.shields.io/badge/WeChat-Группа-07C160?style=flat-square&logo=wechat&logoColor=white)](https://github.com/HKUDS/cognisphereTutor/issues/78)

[Возможности](#-ключевые-возможности) · [Начать](#-начало-работы) · [Исследовать](#-обзор-cognispheretutor) · [CLI](#%EF%B8%8F-cognispheretutor-cli--интерфейс-для-агентов) · [Экосистема](#-экосистема--eduhub-и-сообщество-навыков) · [Сообщество](#-сообщество)

</div>

---

> 🤝 **Мы приветствуем любые вклады в проект!** Голосуйте за элементы дорожной карты или предлагайте новые на странице [`Roadmap`](https://github.com/HKUDS/cognisphereTutor/issues/498), а также изучите наше [Руководство по участию](CONTRIBUTING.md) с описанием стратегии ветвления, стандартов кодирования и инструкций по началу работы.

### 📰 Новости

- **2026-05-22** 🌐 Официальный сайт документации запущен на [**cognispheretutor.info**](https://cognispheretutor.info/) — руководства, справочники и туры по возможностям в одном месте.
...
- **2026-04-10** 📄 Наша статья опубликована на arXiv — прочитайте [препринт](https://arxiv.org/abs/2604.26962) о дизайне и идеях, лежащих в основе cognisphereTutor.
- **2026-02-06** 🚀 10 тысяч звёзд всего за 39 дней! Огромная благодарность нашему удивительному сообществу.
- **2026-01-01** 🎊 С Новым Годом! Присоединяйтесь к нашему [Discord](https://discord.gg/eRsjPgMU4t), [WeChat](https://github.com/HKUDS/cognisphereTutor/issues/78) или [Обсуждениям](https://github.com/HKUDS/cognisphereTutor/discussions) — давайте вместе формировать cognisphereTutor.
- **2025-12-29** 🎓 cognisphereTutor официально выпущен!

## ✨ Ключевые возможности

cognisphereTutor — это агентная учебная рабочая среда, объединяющая репетиторство, решение задач, генерацию викторин, исследования, визуализацию и практику освоения в одной расширяемой системе.
...
cognisphereTutor поставляется с четырьмя путями установки. Все они используют одну структуру рабочего пространства: настройки хранятся в `data/user/settings/` в директории запуска (или в `COGNISPHERETUTOR_HOME` / `cognispheretutor start --home`, если задано явно). Для полного приложения рекомендуемый процесс: **выбрать директорию рабочего пространства → установить → `cognispheretutor init` → `cognispheretutor start`**.

<details>
<summary><b>Вариант 1 — Установка из PyPI</b> · полное локальное веб-приложение + CLI, клонирование не требуется</summary>

Полное локальное веб-приложение + CLI без необходимости клонирования. Требуется **Python 3.11+** и среда выполнения **Node.js 20+** в PATH (упакованный автономный сервер Next.js запускается командой `cognispheretutor start`).

```bash
mkdir -p my-cognispheretutor && cd my-cognispheretutor
pip install -U cognispheretutor
cognispheretutor init     # запрашивает порты + провайдер LLM + необязательное встраивание
cognispheretutor start    # запускает бэкенд + фронтенд; держите терминал открытым
```

`cognispheretutor init` запрашивает порт бэкенда (по умолчанию `8001`), порт фронтенда (по умолчанию `3782`), провайдер LLM / базовый URL / API-ключ / модель и необязательный провайдер встраивания для базы знаний / RAG.

После `cognispheretutor start` откройте URL фронтенда, напечатанный в терминале — по умолчанию [http://127.0.0.1:3782](http://127.0.0.1:3782). Нажмите `Ctrl+C` в этом терминале, чтобы остановить бэкенд и фронтенд. Пропустить `cognispheretutor init` можно для быстрого пробного запуска; приложение загружается с портами по умолчанию и пустыми настройками модели, которые можно настроить позже в **Настройки → Модели**.
...
**Зависший сервер разработки:** если `cognispheretutor start` сообщает о существующем фронтенде, который не отвечает, остановите PID, который он печатает. Если процесс Next.js фактически не запущен, файлы блокировки устарели — удалите их и повторите попытку:
...
- `ghcr.io/hkuds/cognispheretutor:latest` — стабильный выпуск
- `ghcr.io/hkuds/cognispheretutor:pre` — предварительный выпуск, когда доступен
...
Откройте [http://127.0.0.1:3782](http://127.0.0.1:3782). Контейнер создаёт `/app/data/user/settings/*.json` при первом запуске; настройте провайдеров моделей на странице веб-настроек. Конфигурация, API-ключи, логи, файлы рабочего пространства, память и базы знаний сохраняются в томе `cognispheretutor-data`.
...
- **Фоновый режим:** добавьте `-d`, затем `docker logs -f cognispheretutor` для слежения, `docker stop cognispheretutor` для остановки, `docker rm cognispheretutor` перед повторным использованием имени. Том `cognispheretutor-data` сохраняет ваши настройки и рабочее пространство между перезапусками.
...
`next_public_api_base_external` (и его псевдоним `public_api_base`) принимаются как запасные варианты с более низким приоритетом. CORS использует **источники** фронтенда, а не URL API. При отключённой аутентификации cognisphereTutor разрешает обычные HTTP/HTTPS источники браузера по умолчанию. При включённой аутентификации добавьте точные источники фронтенда:
...
Локальная установка `cognispheretutor-cli` не включает веб-ресурсы или серверные зависимости. Сохраните чекаут исходного кода — редактируемая установка указывает на него. Чтобы позже добавить веб-приложение, установите пакет PyPI (Вариант 1) и запустите `cognispheretutor init` + `cognispheretutor start` из того же рабочего пространства.
...
- **docker-compose:** вместо этого направляется в защищённый сайдкар с минимальными привилегиями (**runner sidecar** `Dockerfile.runner`) через `COGNISPHERETUTOR_SANDBOX_RUNNER_URL` — наиболее строгий вариант, используемый автоматически при наличии.

Подпроцессная песочница управляется настройкой `sandbox_allow_subprocess` в `data/user/settings/system.json` (по умолчанию `true`). Запуск сгенерированного моделью кода на вашем хосте — это реальное доверительное решение; установите значение `false` (или экспортируйте `COGNISPHERETUTOR_SANDBOX_ALLOW_SUBPROCESS=0`) для отключения выполнения на стороне хоста, ценой того, что офисные навыки больше не смогут создавать файлы.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/OVERVIEW.png" alt="Главная страница cognisphereTutor — рабочее пространство чата со всеми поверхностями на боковой панели" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/system%20architecture.png" alt="Архитектура системы cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/home/00-overview.png" alt="Рабочее пространство чата cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/chat-agent-loop.png" alt="Цикл агента чата cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/partners/00-partners%20overview.png" alt="Рабочее пространство партнёров cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/partners-architecture.png" alt="Архитектура партнёров cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/myagents/00-overview.png" alt="Рабочее пространство «Мои Агенты» cognisphereTutor" width="900">
</div>

Мои Агенты превращают других агентов в контекст для cognisphereTutor и делают две разные вещи. **Подключите живого агента** — Claude Code или Codex CLI на вашей машине, или одного из ваших Партнёров — и обращайтесь к нему изнутри хода чата: cognisphereTutor фактически *запускает* другого агента и транслирует его работу в панель Activity через инструмент `consult_subagent`. Выберите его чипом агента (или введите `@`) и задайте, сколько раундов может занять обращение.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/co-writer/00-overview.png" alt="Рабочее пространство Co-Writer cognisphereTutor" width="900">
</div>
...
Его определяющая идея — **хирургическое редактирование**: выберите фрагмент и попросите cognisphereTutor переписать, расширить или сократить его. Агент редактирования может опираться на базу знаний или веб-данные, ведёт след вызовов инструментов и показывает каждое изменение как diff с принятием/отклонением — так ничто не применяется до вашего одобрения.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/book/00-book_overview.png" alt="Библиотека книг cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/knowledge/00-overview.png" alt="Центр Знаний cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/learning-space/00-overview.png" alt="Хаб Пространства Обучения cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/00-overview.png" alt="Обзор Памяти cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/01-3%20layer%20memory%20graph.png" alt="Граф Памяти cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/settings/00-setting%20overview.png" alt="Хаб настроек cognisphereTutor" width="900">
</div>
...
Аутентификация **отключена по умолчанию** — cognisphereTutor работает в однопользовательском режиме. Включите её, и одно дерево `data/` содержит рабочее пространство администратора, изолированные пользовательские рабочие пространства и партнёрские рабочие пространства рядом:
...
## ⌨️ cognisphereTutor CLI — Интерфейс для Агентов

Один бинарный файл `cognispheretutor`, два способа входа: интерактивный **REPL** для тех, кто живёт в терминале, и структурированный **JSON** для других агентов, которые управляют cognisphereTutor как инструментом. Одни и те же возможности, инструменты и базы знаний в любом случае.

<details>
<summary><b>Управляйте им самостоятельно</b></summary>

`cognispheretutor chat` открывает интерактивный REPL; `cognispheretutor run <capability> "<message>"` выполняет один ход и завершается. Оба используют одинаковые флаги `--capability`, `--tool`, `--kb` и `--config`.
...
В репозитории есть корневой [`SKILL.md`](SKILL.md) — документ передачи на ~150 строк, который знакомит любой использующий инструменты LLM со всей поверхностью за одно чтение. Передайте его Claude Code, Codex или OpenCode (они автоматически подхватывают `SKILL.md`), или оберните `cognispheretutor run` как инструмент в цикл LangChain / AutoGen. Полные рецепты: [Agent Handoff](https://cognispheretutor.info/docs/cli/agent-handoff/).
...
| Команда | Описание |
|:---|:---|
| `cognispheretutor init` | Создать или обновить `data/user/settings` для текущего рабочего пространства |
| `cognispheretutor start [--home PATH]` | Запустить бэкенд + фронтенд вместе |
| `cognispheretutor serve [--port PORT]` | Запустить только бэкенд FastAPI |
| `cognispheretutor run <capability> <message>` | Запустить один ход возможности (`chat`, `deep_solve`, `deep_question`, `deep_research`, `visualize`, `math_animator`, `mastery_path`); добавьте `--format json` для вывода NDJSON |
| `cognispheretutor chat` | Интерактивный REPL с управлением возможностями, инструментами, KB, блокнотами и историей |
| `cognispheretutor partner list/create/start/stop` | Управление партнёрами, подключёнными к IM |
| `cognispheretutor kb list/info/create/add/search/set-default/delete` | Управление базами знаний LlamaIndex |
| `cognispheretutor skill search/install/list/remove/login/logout/publish/update` | Управление навыками, установка из хабов и публикация своих (`eduhub:<slug>` по умолчанию, см. Экосистема) |
| `cognispheretutor memory show/clear` | Просмотр документов памяти L2/L3 или очистка памяти L1/всей памяти |
| `cognispheretutor session list/show/open/rename/delete` | Управление общими сессиями |
| `cognispheretutor notebook list/create/show/add-md/replace-md/remove-record` | Управление блокнотами из файлов Markdown |
| `cognispheretutor book list/health/refresh-fingerprints` | Просмотр книг и обновление исходных отпечатков |
| `cognispheretutor plugin list/info` | Просмотр зарегистрированных инструментов и возможностей |
| `cognispheretutor config show` | Вывод сводки конфигурации |
| `cognispheretutor provider login <provider>` | Аутентификация провайдера (`openai-codex` вход через OAuth; `github-copilot` проверяет существующую сессию аутентификации Copilot) |
...
Пакет только с CLI находится в `packaging/cognispheretutor-cli`. В этом чекауте установите его из исходного кода:
...
Навыки cognisphereTutor используют открытый формат **Agent-Skills** — папку с плейбуком `SKILL.md` (YAML frontmatter + Markdown) и необязательными справочными файлами. В нём нет ничего специфичного для cognisphereTutor, поэтому любой реестр, говорящий на этом формате, становится источником для вашей библиотеки. cognisphereTutor поставляется с **[EduHub](https://eduhub.cognispheretutor.info/)** — нашим собственным образовательным реестром навыков — встроенным в качестве хаба по умолчанию.

<details>
<summary><b>EduHub — экосистема навыков cognisphereTutor</b></summary>

[**EduHub**](https://eduhub.cognispheretutor.info/) — это хаб сообщества, запущенный cognisphereTutor для обмена обучающими навыками агентов — сократовские репетиторы, создатели флэш-карточек, обратная связь по эссе, планы экзаменов, объяснители концепций и многое другое. Он встроен в cognisphereTutor, поэтому настраивать ничего не нужно: голый слаг или префикс `eduhub:` разрешается в него.
...
```bash
cognispheretutor skill search "socratic tutor"               # поиск в EduHub (хаб по умолчанию)
cognispheretutor skill install socratic-tutor                # получить → проверить → зарегистрировать
cognispheretutor skill install eduhub:socratic-tutor@1.2.0   # указать хаб и версию
cognispheretutor skill list                                  # локальные навыки с их хабовой принадлежностью
```

**Опубликуйте свой** — упакуйте `SKILL.md` и поделитесь им обратно с сообществом:

```bash
cognispheretutor skill login                                 # вход через браузер в EduHub
cognispheretutor skill publish ./my-skill                    # интерактивно: выберите трек + теги, затем загрузите
cognispheretutor skill update                                # откатиться или выпустить новую версию
```

EduHub также является отдельным, совместимым с ClawHub реестром, поэтому агенты, отличные от cognisphereTutor (Claude Code, Codex, …), могут использовать его напрямую через CLI `eduhub` — `npx eduhub install socratic-tutor`.
...
- frontmatter нормализуется по схеме cognisphereTutor, и `always:` **удаляется**, поэтому загруженный навык никогда не может принудить себя в каждый системный промпт;
...
Поскольку cognisphereTutor использует открытый формат Agent-Skills, **[ClawHub](https://clawhub.ai/)** также работает как первоклассный источник — он встроен рядом с EduHub. Выберите его с префиксом хаба:
...
| [**Claude Code**](https://github.com/anthropics/claude-code) | Агентный CLI кодирования, вдохновивший цикл агента cognisphereTutor |
...
Мы хотим, чтобы cognisphereTutor продолжал развиваться и совершенствоваться — и в конечном итоге стал подарком, который мы возвращаем сообществу открытого исходного кода. Наша [**дорожная карта**](https://github.com/HKUDS/cognisphereTutor/issues/498) обновляется непрерывно; голосуйте там за пункты или предлагайте новые. Если вы хотите участвовать, см. [**Руководство по участию**](CONTRIBUTING.md) с описанием стратегии ветвления, стандартов кодирования и инструкций по началу работы.

<div align="center">

Мы надеемся, что cognisphereTutor станет подарком для сообщества. 🎁

<a href="https://github.com/HKUDS/cognisphereTutor/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=HKUDS/cognisphereTutor&max=999" alt="Участники" />
</a>
...
<a href="https://www.star-history.com/#HKUDS/cognisphereTutor&type=timeline&legend=top-left">
  <picture>
    ...
    <img alt="Диаграмма истории звёзд" src="https://api.star-history.com/svg?repos=HKUDS/cognisphereTutor&type=timeline&legend=top-left" />
  </picture>
</a>
...
<p align="center">
 <a href="https://www.star-history.com/hkuds/cognispheretutor">
  <picture>
   ...
   <img alt="Рейтинг истории звёзд" src="https://api.star-history.com/badge?repo=HKUDS/cognisphereTutor" />
  </picture>
 </a>
</p>
