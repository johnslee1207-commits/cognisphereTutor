...
# cognisphereTutor: Dożywotnie Spersonalizowane Korepetycje
...
[Funkcje](#-główne-funkcje) · [Zacznij](#-pierwsze-kroki) · [Eksploruj](#-eksploracja-cognispheretutor) · [CLI](#️-cognispheretutor-cli--interfejs-natywny-dla-agentów) · [Ekosystem](#-ekosystem--eduhub-i-społeczność-umiejętności) · [Społeczność](#-społeczność)

</div>

---

> 🤝 **Zapraszamy do wszelkich form współpracy!** Głosuj na elementy planu działania lub proponuj nowe w [`Roadmap`](https://github.com/HKUDS/cognisphereTutor/issues/498), a szczegóły dotyczące strategii gałęzi, standardów kodowania i sposobu rozpoczęcia pracy znajdziesz w naszym [Przewodniku dla współtwórców](../../CONTRIBUTING.md).

### 📰 Aktualności

- **2026-05-22** 🌐 Oficjalna strona dokumentacji dostępna na [**cognispheretutor.info**](https://cognispheretutor.info/) — przewodniki, odniesienia i wycieczki po możliwościach w jednym miejscu.
...
- **2026-04-10** 📄 Nasz artykuł jest już dostępny na arXiv — przeczytaj [preprint](https://arxiv.org/abs/2604.26962), aby dowiedzieć się więcej o projekcie i pomysłach stojących za cognisphereTutor.
- **2026-02-06** 🚀 10 tys. gwiazdek w zaledwie 39 dni! Ogromne podziękowania dla naszej niesamowitej społeczności.
- **2026-01-01** 🎊 Szczęśliwego Nowego Roku! Dołącz do naszego [Discorda](https://discord.gg/eRsjPgMU4t), [WeChat](https://github.com/HKUDS/cognisphereTutor/issues/78) lub [Dyskusji](https://github.com/HKUDS/cognisphereTutor/discussions) — razem kształtujmy przyszłość cognisphereTutor.
- **2025-12-29** 🎓 cognisphereTutor jest oficjalnie wydany!

## ✨ Główne funkcje

cognisphereTutor to natywne dla agentów środowisko nauki, które łączy korepetycje, rozwiązywanie problemów, generowanie quizów, badania, wizualizacje i ćwiczenia opanowania wiedzy w jednym rozszerzalnym systemie.
...
cognisphereTutor oferuje cztery ścieżki instalacji. Wszystkie współdzielą jeden układ obszaru roboczego: ustawienia żyją w `data/user/settings/` pod katalogiem, z którego uruchamiasz (lub pod `COGNISPHERETUTOR_HOME` / `cognispheretutor start --home` jeśli ustawisz je jawnie). Dla pełnej aplikacji zalecany przepływ to **wybierz katalog obszaru roboczego → zainstaluj → `cognispheretutor init` → `cognispheretutor start`**.

<details>
<summary><b>Opcja 1 — Instalacja z PyPI</b> · pełna lokalna aplikacja Web + CLI, bez potrzeby klonowania</summary>

Pełna lokalna aplikacja Web + CLI, bez potrzeby klonowania. Wymaga **Python 3.11+** i środowiska uruchomieniowego **Node.js 20+** na PATH (spakowany serwer standalone Next.js jest uruchamiany przez `cognispheretutor start`).

```bash
mkdir -p my-cognispheretutor && cd my-cognispheretutor
pip install -U cognispheretutor
cognispheretutor init     # pyta o porty + dostawcę LLM + opcjonalne osadzanie
cognispheretutor start    # uruchamia backend + frontend; utrzymuj terminal otwarty
```

`cognispheretutor init` pyta o port backendu (domyślnie `8001`), port frontendu (domyślnie `3782`), dostawcę LLM / bazowy URL / klucz API / model i opcjonalnego dostawcę osadzania dla Bazy wiedzy / RAG.

Po `cognispheretutor start` otwórz adres URL frontendu wydrukowany w terminalu — domyślnie [http://127.0.0.1:3782](http://127.0.0.1:3782). Naciśnij `Ctrl+C` w tym terminalu, aby zatrzymać backend i frontend. Pominięcie `cognispheretutor init` jest dobre dla szybkiego testu; aplikacja uruchamia się z domyślnymi portami i pustymi ustawieniami modelu, skonfiguruj je później w **Settings → Models**.
...
**Zablokowany serwer deweloperski:** jeśli `cognispheretutor start` zgłasza istniejący frontend, który nie odpowiada, zatrzymaj PID, który wydrukuje. Jeśli żaden proces Next.js nie jest uruchomiony, pliki blokady są przestarzałe — usuń je i spróbuj ponownie:
...
- `ghcr.io/hkuds/cognispheretutor:latest` — stabilne wydanie
- `ghcr.io/hkuds/cognispheretutor:pre` — wersja wstępna, gdy dostępna
...
Otwórz [http://127.0.0.1:3782](http://127.0.0.1:3782). Kontener tworzy `/app/data/user/settings/*.json` przy pierwszym uruchomieniu; skonfiguruj dostawców modeli ze strony Web Settings. Konfiguracja, klucze API, dzienniki, pliki obszaru roboczego, pamięć i bazy wiedzy są przechowywane w woluminie `cognispheretutor-data`.
...
- **Odłączony:** dodaj `-d`, następnie `docker logs -f cognispheretutor`, aby śledzić, `docker stop cognispheretutor`, aby zatrzymać, `docker rm cognispheretutor` przed ponownym użyciem nazwy. Wolumin `cognispheretutor-data` przechowuje ustawienia i obszar roboczy między restartami.
...
`cognispheretutor init --cli` współdzieli ten sam układ `data/user/settings/` co pełna aplikacja, ale pomija monity o porty backendu/frontendu i domyślnie wyłącza osadzanie (wybierz `Yes` jeśli planujesz używać `cognispheretutor kb …` lub narzędzi RAG). Nadal zapisuje kompletny układ środowiska uruchomieniowego (`system.json`, `auth.json`, `integrations.json`, `model_catalog.json`, `main.yaml`, `agents.yaml`) i nadal pyta o aktywnego dostawcę LLM i model.

<details>
<summary><b>Typowe polecenia</b></summary>

```bash
cognispheretutor chat                                          # interaktywny REPL
...
Lokalna instalacja `cognispheretutor-cli` nie zawiera zasobów webowych ani zależności serwera. Zachowaj kod źródłowy — instalacja edytowalna wskazuje na niego. Aby dodać aplikację Web później, zainstaluj pakiet PyPI (Opcja 1) i uruchom `cognispheretutor init` + `cognispheretutor start` z tego samego obszaru roboczego.
...
- **docker-compose:** zamiast tego kierowany do utwardzonego, o najmniejszych uprawnieniach **sidecar runner** (`Dockerfile.runner`) przez `COGNISPHERETUTOR_SANDBOX_RUNNER_URL` — najsilniejsza postawa, automatycznie preferowana gdy jest obecna.

Piaskownica podprocesu jest kontrolowana przez ustawienie `sandbox_allow_subprocess` w `data/user/settings/system.json` (domyślnie `true`). Uruchamianie kodu generowanego przez model na twoim hoście to prawdziwa decyzja dotycząca zaufania — ustaw na `false` (lub eksportuj `COGNISPHERETUTOR_SANDBOX_ALLOW_SUBPROCESS=0`), aby wyłączyć wykonanie po stronie hosta kosztem tego, że umiejętności biurowe nie będą mogły produkować plików.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/OVERVIEW.png" alt="Strona główna cognisphereTutor — obszar roboczy Chat z każdą powierzchnią w pasku bocznym" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/system%20architecture.png" alt="Architektura systemu cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/home/00-overview.png" alt="Obszar roboczy czatu cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/chat-agent-loop.png" alt="Pętla agenta czatu cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/partners/00-partners%20overview.png" alt="Obszar roboczy Partners cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/partners-architecture.png" alt="Architektura Partners cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/myagents/00-overview.png" alt="Obszar roboczy My Agents cognisphereTutor" width="900">
</div>

My Agents zamienia innych agentów w kontekst dla cognisphereTutor i wykonuje dwie odrębne rzeczy. **Połącz żywego agenta** — Claude Code lub Codex CLI na twoim komputerze, lub jednego z twoich Partners — i konsultuj go z wnętrza tury czatu: cognisphereTutor faktycznie *uruchamia* drugiego agenta i strumieniuje jego pracę do panelu Activity przez narzędzie `consult_subagent`. Wybierz go chipem Agent (lub wpisz `@`) i ustaw ile rund może trwać konsultacja.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/co-writer/00-overview.png" alt="Obszar roboczy Co-Writer cognisphereTutor" width="900">
</div>
...
Jego wyróżniającym pomysłem jest **precyzyjna edycja**: zaznacz fragment i poproś cognisphereTutor o przepisanie, rozszerzenie lub skrócenie. Agent edycji może ugruntować zmianę w bazie wiedzy lub dowodach webowych, zachowuje ślad swoich wywołań narzędzi i pokazuje każdą zmianę jako diff akceptuj/odrzuć — więc nic nie ląduje dopóki tego nie zatwierdzisz.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/book/00-book_overview.png" alt="Biblioteka książek cognisphereTutor" width="900">
</div>
...
Każdy rozdział kompiluje się do typowanych bloków — tekst, callouts, quizy, fiszki, osie czasu, kod, figury, interaktywny HTML, animacje, grafy konceptów, dogłębne analizy i notatki użytkownika — a każda strona ma swój własny Page Chat. Bloki są edytowalne: wstawiaj, przenoś, regeneruj lub zmieniaj typ bloku bez przepisywania rozdziału. Polecenia konserwacyjne takie jak `cognispheretutor book health` i `cognispheretutor book refresh-fingerprints` pomagają wykryć kiedy wiedza źródłowa odbiega od skompilowanych stron.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/knowledge/00-overview.png" alt="Knowledge Center cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/learning-space/00-overview.png" alt="Centrum Learning Space cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/00-overview.png" alt="Przegląd Memory cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/01-3%20layer%20memory%20graph.png" alt="Graf pamięci cognisphereTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/settings/00-setting%20overview.png" alt="Centrum ustawień cognisphereTutor" width="900">
</div>
...
Uwierzytelnianie jest **domyślnie wyłączone** — cognisphereTutor działa jednoosobowo. Włącz je a jedno drzewo `data/` obsługuje obszar roboczy administratora, izolowane obszary robocze per-użytkownik i obszary robocze partnerów obok siebie:
...
**Włącz:** włącz uwierzytelnianie w `data/user/settings/auth.json`, uruchom ponownie `cognispheretutor start`, zarejestruj pierwszego administratora pod `/register`, następnie dodaj użytkowników z `/admin/users` i przypisz modele, KB, umiejętności, Partners, politykę narzędzi/MCP i dostęp do wykonania kodu przez uprawnienia.
...
## ⌨️ cognisphereTutor CLI — Interfejs natywny dla agentów

Jeden plik binarny `cognispheretutor`, dwa sposoby wejścia: interaktywny **REPL** dla osób żyjących w terminalu i strukturalny **JSON** dla innych agentów które prowadzą cognisphereTutor jako narzędzie. Te same możliwości, narzędzia i bazy wiedzy w obu przypadkach.

<details>
<summary><b>Prowadź sam</b></summary>

`cognispheretutor chat` otwiera interaktywny REPL; `cognispheretutor run <capability> "<message>"` uruchamia jedną turę i wychodzi. Oba używają tych samych flag `--capability`, `--tool`, `--kb` i `--config`.
...
| Polecenie | Opis |
|:---|:---|
| `cognispheretutor init` | Utwórz lub zaktualizuj `data/user/settings` dla bieżącego obszaru roboczego |
| `cognispheretutor start [--home PATH]` | Uruchom backend + frontend razem |
| `cognispheretutor serve [--port PORT]` | Uruchom tylko backend FastAPI |
| `cognispheretutor run <capability> <message>` | Uruchom jedną turę możliwości (`chat`, `deep_solve`, `deep_question`, `deep_research`, `visualize`, `math_animator`, `mastery_path`); dodaj `--format json` dla wyjścia NDJSON |
| `cognispheretutor chat` | Interaktywny REPL z kontrolkami możliwości, narzędzia, KB, notatnika i historii |
| `cognispheretutor partner list/create/start/stop` | Zarządzaj partnerami połączonymi przez IM |
| `cognispheretutor kb list/info/create/add/search/set-default/delete` | Zarządzaj bazami wiedzy LlamaIndex |
| `cognispheretutor skill search/install/list/remove/login/logout/publish/update` | Zarządzaj umiejętnościami, instaluj z hubów i publikuj własne (`eduhub:<slug>` domyślnie, patrz Ekosystem) |
| `cognispheretutor memory show/clear` | Inspekcjonuj dokumenty pamięci L2/L3 lub wyczyść pamięć L1/wszystko |
| `cognispheretutor session list/show/open/rename/delete` | Zarządzaj współdzielonymi sesjami |
| `cognispheretutor notebook list/create/show/add-md/replace-md/remove-record` | Zarządzaj notatnikami z plików Markdown |
| `cognispheretutor book list/health/refresh-fingerprints` | Inspekcjonuj książki i odświeżaj odciski źródeł |
| `cognispheretutor plugin list/info` | Inspekcjonuj zarejestrowane narzędzia i możliwości |
| `cognispheretutor config show` | Wydrukuj podsumowanie konfiguracji |
| `cognispheretutor provider login <provider>` | Uwierzytelnianie dostawcy (`openai-codex` logowanie OAuth; `github-copilot` weryfikuje istniejącą sesję auth Copilot) |
...
Pakiet tylko CLI żyje w `packaging/cognispheretutor-cli`. W tym kodzie źródłowym zainstaluj go ze źródła:
...
Umiejętności cognisphereTutor używają otwartego formatu **Agent-Skills** — folder z podręcznikiem `SKILL.md` (frontmatter YAML + Markdown) i opcjonalnymi plikami referencyjnymi. Nic w tym nie jest specyficzne dla cognisphereTutor, więc każdy rejestr który mówi w tym formacie staje się źródłem dla twojej biblioteki. cognisphereTutor jest dostarczany z **[EduHub](https://eduhub.cognispheretutor.info/)** — naszym własnym rejestrem umiejętności skupionym na edukacji — podłączonym jako domyślny hub.

<details>
<summary><b>EduHub — Ekosystem umiejętności cognisphereTutor</b></summary>

[**EduHub**](https://eduhub.cognispheretutor.info/) to hub społecznościowy który cognisphereTutor uruchomił do dzielenia się umiejętnościami agentów zorientowanymi na nauczanie — tutorzy sokratejscy, kreatory fiszek, informacje zwrotne na eseje, plany egzaminów, objaśniacze konceptów i więcej. Jest wbudowany w cognisphereTutor, więc nie ma nic do konfigurowania: gołe slug lub prefiks `eduhub:` rozwiązuje do niego.
...
```bash
cognispheretutor skill search "socratic tutor"               # wyszukaj EduHub (domyślny hub)
cognispheretutor skill install socratic-tutor                # pobierz → weryfikuj → zarejestruj
cognispheretutor skill install eduhub:socratic-tutor@1.2.0   # przypnij hub i wersję
cognispheretutor skill list                                  # lokalne umiejętności z proweniencją hubu
```

**Opublikuj własną** — spakuj `SKILL.md` i podziel się ze społecznością:

```bash
cognispheretutor skill login                                 # logowanie przeglądarki do EduHub
cognispheretutor skill publish ./my-skill                    # interaktywne: wybierz ścieżkę + tagi, następnie prześlij
cognispheretutor skill update                                # wycofaj lub wydaj nową wersję
```

EduHub jest również samodzielnym, kompatybilnym z ClawHub rejestrem, więc agenty które nie są cognisphereTutorem (Claude Code, Codex, …) mogą używać go bezpośrednio przez CLI `eduhub` — `npx eduhub install socratic-tutor`.
...
- frontmatter jest normalizowany do schematu cognisphereTutor a `always:` jest **usuwane**, więc pobrana umiejętność nigdy nie może wymusić się do każdego systemowego promptu;
...
Ponieważ cognisphereTutor mówi otwartym formatem Agent-Skills, **[ClawHub](https://clawhub.ai/)** działa również jako pierwszorzędne źródło — jest wbudowany obok EduHub. Wybierz go z prefiksem hubu:
...
| [**Claude Code**](https://github.com/anthropics/claude-code) | Agentowy CLI kodowania który zainspirował pętlę agenta cognisphereTutor |
...
Chcemy aby cognisphereTutor stale iterował i się rozwijał — a ostatecznie stał się prezentem który oddajemy społeczności open-source. Nasz [**plan działania**](https://github.com/HKUDS/cognisphereTutor/issues/498) jest aktualizowany na bieżąco; głosuj na elementy tam lub proponuj nowe. Jeśli chcesz współtworzyć, zapoznaj się z [**Przewodnikiem dla współtwórców**](../../CONTRIBUTING.md) po strategię gałęzi, standardy kodowania i sposób rozpoczęcia.

<div align="center">

Mamy nadzieję, że cognisphereTutor stanie się prezentem dla społeczności. 🎁

<a href="https://github.com/HKUDS/cognisphereTutor/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=HKUDS/cognisphereTutor&max=999" alt="Współtwórcy" />
</a>
...
<a href="https://www.star-history.com/#HKUDS/cognisphereTutor&type=timeline&legend=top-left">
  <picture>
    ...
    <img alt="Wykres historii gwiazdek" src="https://api.star-history.com/svg?repos=HKUDS/cognisphereTutor&type=timeline&legend=top-left" />
  </picture>
</a>
...
<p align="center">
 <a href="https://www.star-history.com/hkuds/cognispheretutor">
  <picture>
   ...
   <img alt="Ranking historii gwiazdek" src="https://api.star-history.com/badge?repo=HKUDS/cognisphereTutor" />
  </picture>
 </a>
</p>
...
<p>
  <img src="https://visitor-badge.laobi.icu/badge?page_id=HKUDS.cognisphereTutor&style=for-the-badge&color=00d4ff" alt="Odwiedziny">
</p>
