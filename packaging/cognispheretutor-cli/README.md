...
CLI-only cognisphereTutor distribution. It installs the `cognispheretutor` command and the
...
FastAPI/Uvicorn server dependencies used by `cognispheretutor start`.
...
the `cognispheretutor` command at these source files.
